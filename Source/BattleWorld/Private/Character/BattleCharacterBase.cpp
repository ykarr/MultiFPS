...
void ABattleCharacterBase::SRV_DeathDropWeapon_Implementation()
{
	...
	GetWorld()->GetTimerManager().SetTimer(WeaponDropResetDelay, [this, &WeaponDropResetDelay]() {
		if (!WeaponInventory.IsEmpty()) {
			for (TObjectPtr<AWeaponBase> Weapon : WeaponInventory) {
				DropWeapon(Weapon);
				FTimerHandle WeaponDropResetDelay2;
				GetWorld()->GetTimerManager().SetTimer(WeaponDropResetDelay2, [this, &WeaponDropResetDelay]() {
